...
void ConnectionStatus_Update(void)
{
    ...
    bool mqtt_status = false;  // MQTT功能已禁用
...
/**
 * @brief MQTT初始化（参考e1_iot_smart_home）- 已禁用
 */
/*
void mqtt_init(void)
{
...
/**
 * @brief IoT网络任务实现（参考e1_iot_smart_home）- 已禁用
 */
/*
...
int IoTCloud_SendData(const LandslideIotData *data)
{
    ...
    if (ConnectionStatus_IsStable() && mqttConnectFlag) {
        ...
        // 然后发送当前数据（减少日志输出）
        // send_msg_to_mqtt(&iot_data);  // MQTT功能已禁用
        printf(" MQTT功能已禁用，数据未发送\n");
...
/**
 * @brief 处理语音模块控制命令 - 已禁用
 * @param enable 是否启用语音模块
 */
void IoTCloud_HandleVoiceCommand(bool enable)
{
    printf("Voice module command ignored (disabled): %s\n", enable ? "ENABLE" : "DISABLE");
    g_cloud_voice_enabled = false;  // 强制禁用

    // 语音模块已禁用，GPIO0_PB2/PB3释放给L610使用
    printf("Voice module permanently disabled (L610 mode)\n");
}

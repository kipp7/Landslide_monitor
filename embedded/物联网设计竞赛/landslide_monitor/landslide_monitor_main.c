...
#include "l610_module.h"  // L610 4G通信模块
...
static UINT32 g_data_upload_thread_id = 0;  // L610数据上传任务
...
static void DataUploadTask(void);  // L610数据上传任务
...
int LandslideMonitorStart(void)
{
    ...
    printf("System startup voice disabled (L610 mode)\n");
...
static int InitializeHardware(void)
{
    ...
    // }
    printf("WiFi+MQTT cloud connection disabled (using L610 4G instead)\n");
    ...
    // 初始化L610 4G通信模块
    ret = L610_Init();
    if (ret != L610_RESULT_SUCCESS) {
        printf("L610 4G module initialization failed: %d (continuing without 4G)\n", ret);
        // L610失败不影响系统运行，但会影响数据上传
    } else {
        printf("L610 4G module initialized successfully\n");

        // 启动L610后台任务
        ret = L610_StartTask();
        if (ret != L610_RESULT_SUCCESS) {
            printf("L610 background task start failed: %d\n", ret);
        } else {
            printf("L610 background task started successfully\n");
        }
    }
...
static int CreateTasks(void)
{
    ...
    // 创建L610数据上传任务
    memset(&task_param, 0, sizeof(task_param));
    task_param.pfnTaskEntry = (TSK_ENTRY_FUNC)DataUploadTask;
    task_param.uwStackSize = THREAD_STACK_SIZE;
    task_param.pcName = "DataUploadTask";
    task_param.usTaskPrio = THREAD_PRIO_ALARM + 1;  // 比报警任务优先级稍低
    ret = LOS_TaskCreate(&g_data_upload_thread_id, &task_param);
    if (ret != LOS_OK) {
        printf("Failed to create data upload task: %d (continuing without upload)\n", ret);
        // 数据上传任务失败不影响系统运行
    } else {
        printf("Data upload task created successfully\n");
    }
    ...
    while (g_system_state == SYSTEM_STATE_RUNNING || g_system_state == SYSTEM_STATE_WARNING) {
        ...
        if (current_time - last_voice_time >= VOICE_REPORT_INTERVAL_S * 1000) {
            ...
            if (assessment.level >= RISK_LEVEL_LOW) {
                printf("Voice alert disabled: Risk level %d\n", assessment.level);
            } else {
                printf("Voice alert disabled: Status safe\n");
            }
 ...
 * @brief L610数据上传任务
 */
static void DataUploadTask(void)
{
    LandslideIotData upload_data;
    uint32_t last_upload_time = 0;
    uint32_t upload_interval = 30000;  // 30秒上传间隔
    L610_Stats l610_stats;

    printf("Data upload task started\n");
    ...
    while (g_system_state == SYSTEM_STATE_RUNNING || g_system_state == SYSTEM_STATE_WARNING) {
        ...
        if (current_time - last_upload_time >= upload_interval) {
            ...
            if (L610_IsConnected()) {
                // 获取最新的传感器数据
                if (LOS_MuxPend(g_data_mutex, LOS_WAIT_FOREVER) == LOS_OK) {
                    // 构建上传数据
                    memset(&upload_data, 0, sizeof(upload_data));

                    // 基础传感器数据
                    upload_data.temperature = g_latest_sensor_data.sht_temperature;
                    upload_data.humidity = g_latest_sensor_data.humidity;
                    upload_data.light = g_latest_sensor_data.light_intensity;

                    // MPU6050数据
                    upload_data.accel_x = g_latest_sensor_data.accel_x;
                    upload_data.accel_y = g_latest_sensor_data.accel_y;
                    upload_data.accel_z = g_latest_sensor_data.accel_z;
                    upload_data.gyro_x = g_latest_sensor_data.gyro_x;
                    upload_data.gyro_y = g_latest_sensor_data.gyro_y;
                    upload_data.gyro_z = g_latest_sensor_data.gyro_z;

                    // GPS数据
                    upload_data.gps_latitude = g_latest_sensor_data.gps_latitude;
                    upload_data.gps_longitude = g_latest_sensor_data.gps_longitude;

                    // 处理后的数据
                    upload_data.vibration = g_latest_processed_data.vibration_intensity;
                    upload_data.angle_x = g_latest_sensor_data.angle_x;  // 从传感器数据获取
                    upload_data.angle_y = g_latest_sensor_data.angle_y;  // 从传感器数据获取

                    // 风险评估数据
                    upload_data.risk_level = g_latest_risk_assessment.level;
                    upload_data.alarm_active = (g_latest_risk_assessment.level >= RISK_LEVEL_HIGH);

                    // 系统状态
                    upload_data.uptime = current_time / 1000;  // 转换为秒
                    upload_data.gps_valid = g_latest_sensor_data.gps_valid;

                    LOS_MuxPost(g_data_mutex);

                    // 上传数据
                    printf("Uploading data via L610...\n");
                    L610_Result result = L610_UploadLandslideData(&upload_data);

                    if (result == L610_RESULT_SUCCESS) {
                        printf("✅ Data uploaded successfully via L610\n");
                        last_upload_time = current_time;
                    } else {
                        printf("❌ Data upload failed: %d\n", result);
                        // 上传失败时缩短重试间隔
                        upload_interval = 10000;  // 10秒后重试
                    }

                    // 获取并显示L610统计信息
                    L610_GetStats(&l610_stats);
                    uint32_t total_uploads = l610_stats.upload_success_count + l610_stats.upload_error_count;
                    if (total_uploads > 0) {
                        printf("L610 Stats: %d/%d uploads successful (%.1f%%)\n",
                               l610_stats.upload_success_count, total_uploads,
                               (float)l610_stats.upload_success_count * 100.0f / total_uploads);
                    }
                } else {
                    printf("Failed to acquire data mutex for upload\n");
                }
            } else {
                printf("L610 not ready for upload\n");
                // L610未就绪时延长检查间隔
                upload_interval = 60000;  // 60秒后重试
            }

            // 恢复正常上传间隔
            if (upload_interval != 30000) {
    ...
    printf("Data upload task stopped\n");
}
